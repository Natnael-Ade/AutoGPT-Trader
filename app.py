...
class TradingAnalyzer:
    ...
    def fetch_stock_data(self, ticker, period="1mo"):
        """Fetch stock data with better error handling and fallback to sample data"""
        try:
    ...
    def _get_sample_stock_data(self, ticker):
        ...
        # Generate sample prices with some randomness but following a trend
        base_price = 150.0 if ticker in ['AAPL', 'MSFT'] else 100.0
        trend = 0.005  # Slight upward trend

        # Create sample data
        data = pd.DataFrame(index=dates)
        data['Close'] = [base_price * (1 + trend * i + 0.01 * np.random.randn()) for i in range(len(dates))]

        # Create Open prices with proper pandas indexing
        open_prices = data['Close'].shift(1) * (1 + 0.005 * np.random.randn())
        data['Open'] = open_prices

        # Set the first Open price properly using loc
        data.loc[data.index[0], 'Open'] = data['Close'].iloc[0] * 0.99

        # High is the max of open and close plus some random value
        data['High'] = np.maximum(data['Open'], data['Close']) * (1 + 0.01 * np.random.rand(len(dates)))

        # Low is the min of open and close minus some random value
        data['Low'] = np.minimum(data['Open'], data['Close']) * (1 - 0.01 * np.random.rand(len(dates)))

        # Generate some realistic looking volume
        ...
        if self.USE_OPENAI:
            ...
            try:
                if os.getenv("GEMINI_API_KEY"):
                    ...
                    - Risk Level: {risk_level}
                    - Market Sentiment: NEUTRAL (Score: 0.00)
