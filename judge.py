...
# Choose AI model
USE_OPENAI = st.sidebar.radio("Select AI Model", ["OpenAI", "Gemini"], index=1) == "OpenAI"
if USE_OPENAI:
    import openai
    ...
    try:
        ...
        if not gemini_api_key:
            st.error("Gemini API key not found in environment variables. Falling back to OpenAI.")
            USE_OPENAI = True
        else:
            genai.configure(api_key=gemini_api_key)
    except (ImportError, Exception) as e:
        st.error(f"Error configuring Gemini: {str(e)}. Falling back to OpenAI.")
        USE_OPENAI = True
...
class TradingChatAssistant:
    ...
    def fetch_stock_data(self, ticker, period="1mo"):
        ...
        try:
            ...
            st.error(f"Network error fetching data for {ticker}: {str(e)}")
            return None
    ...
    def get_news(self, query, hours=24, limit=5):
        ...
        try:
            ...
            if response.get('status') != 'ok':
                st.warning("News API error - using sample data")
                return self._get_sample_news()
    ...
    def get_ai_suggestion(self, ticker, data, news, past_memories):
        ...
        indicator_alignment = sum([
            1 if current_rsi > 50 and price_trend == 'Upward' else -1 if current_rsi < 50 and price_trend == 'Downward' else 0,
    ...
    def generate_chat_response(self, user_input):
        """Generate a response to user input in chat format"""
        if not user_input or not isinstance(user_input, str):
            return "Please enter a valid question about a stock"

        ticker = self._extract_ticker(user_input)
        if not ticker:
            return "Please specify a stock ticker/symbol (e.g., 'What about AAPL?' or 'Analyze TSLA')"
...
**Technical Analysis Summary:**
- Current Price: ${latest_close:.2f}
- 20-day SMA: {sma_20_str} ({sma_20_comparison})
- 50-day SMA: {sma_50_str} ({sma_50_comparison})
- RSI (14): {rsi_str}
- Support Level: ${support:.2f}
- Resistance Level: ${resistance:.2f}
...
**AI Analysis:**
{ai_rec if ai_rec else 'No AI analysis available'}
"""
            return response
...
def main():
    ...
    st.write("Chat with our AI to get BUY/SELL/HOLD recommendations based on technical and sentiment analysis")
    ...
    # Accept user input
    if prompt := st.chat_input("Ask about a stock (e.g., 'What about AAPL?' or 'Should I buy TSLA?')"):
        ...
        response = assistant.generate_chat_response(prompt)
